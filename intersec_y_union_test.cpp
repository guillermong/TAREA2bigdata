...
int main(int argc, char **argv) {
	...
	float tiempo=0,tbuscar,tunion,tinterseccion,tfilas;
	clock_t t1;
	map <string  ,vector<int> > indice;
	string buf ,palabra1,palabra2;;
	int pos=0,posting=0,k,i1=0,i2=0,f1,f2,rep;
	char r[20];
	...
	*/	  	
	...
	//************UNION E INTERSECCION PALABRAS*******************
		...
		for (k=8;k<=16;k++){
			p = pow(2,k);
			tunion=0;tinterseccion=0;tfilas=0;
			for(pos=1; pos<=p ;pos++){
				...
				t1 = clock()-t1;
				tfilas += ((float)t1)/CLOCKS_PER_SEC;
		...
		//IMPRIMIR TIEMPOS EN K
			cout<<"PARA REPETICIONES="<<rep<<endl;				
			cout<<"PARA K="<<k<<endl;
			cout<<"TIEMPO BUSCAR 2LISTAS:"<<tfilas<<endl;
	...
	while(1){
		...
		f1=lista1.size(),f2=lista2.size();
					...
					cout<<endl;
					buscar=indice[palabra2];
					cout <<"buscar:"<<palabra2<<endl;
					for(unsigned int i = 0; i<buscar.size(); i++) {
						cout << buscar[i] <<" ";
					} 
					cout<<endl;
					...
					unio=funio(i1,f1,i2,f2,lista1,lista2);
					cout <<"UNION:"<<palabra1<<" y "<<palabra2<<endl;
					for(unsigned int i = 0; i<unio.size(); i++) {
						cout << unio[i] <<" ";
					} 
					cout<<endl;
					...
					inter=finterseccion(i1,f1,i2,f2,lista1,lista2);
					cout <<"INTERSECCION:"<<palabra1<<" y "<<palabra2<<endl;
						for(unsigned int i = 0; i<inter.size(); i++) {
							cout << inter[i] <<" ";
						} 
					cout<<endl;
